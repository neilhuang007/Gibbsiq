...
tends towards $N$ when the samples are iid. We do not need the $\operatorname{Var}[x]$ term from (3) because the samples have been standardized using the ground-truth samples' scale. We do not need the $\operatorname{Var}[x]$ term from (3) because the samples have been standardized using the ground-truth samples' scale.
