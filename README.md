...
2. Core implemented (2026-07-01): a THRML optimization runtime that lowers this representation into THRML nodes, factors, graph-colored blocks, sampling programs, schedules, seeds, initialization policies, and trace capture, validated by exhaustive small-instance comparison against analytic Boltzmann probabilities. Opt-in parallel tempering exists, but the 2026-07-11 corrective audit keeps that exit criterion open pending targeted invariants and the full optional THRML suite.
3. Implemented, under corrective semantic audit (2026-07-11): a diagnostics layer that reports autocorrelation and integrated autocorrelation time, raw-energy effective sample size, plain and rank-normalized between-chain disagreement, diversity, and explicit statuses for each THRML run. The existing formulas retain their arviz and R-`posterior` cross-checks. The corrective patch removes the unsupported raw-ESS threshold, renames occupancy efficiency accurately, and separates observations from failures; full-suite verification remains pending. Rank-normalized bulk/tail ESS and constraint feasibility remain future work.
...
Gibbsiq does not currently claim that THRML is faster than classical or GPU baselines, and it does not treat R-hat, ESS, or diversity as proof of optimality. The implemented scope comprises conversion, fixed-beta runtime lowering, graph-colored blocks, schedule and seed controls, raw multi-chain traces, diagnostics, and witness-verified benchmark scoring. Parallel tempering remains under corrective verification, and diagnostic threshold/flag semantics remain under corrective audit. The inspector, constraint-encoding, and classical-baseline layers remain to be built.
